...
app.controller("DonHangCtrl", function ($scope, $http){ //Bộ điều khiển AngularJS điều khiển các ứng dụng:
    // $scope.maDanhMuc 
    total = function(){
        var orderDetails = JSON.parse(localStorage.getItem('cart'));
        let t = 0;
   ...
   $scope.loadCart = function () {
        $scope.list = JSON.parse(localStorage.getItem('cart'));
   };
   $scope.toTal;
   $scope.getTotalPrice = function() {
    let total = 0;
    for (x in JSON.parse(localStorage.getItem('cart'))) {
        total += x.gia * x.soLuong;
    }
    return total;
